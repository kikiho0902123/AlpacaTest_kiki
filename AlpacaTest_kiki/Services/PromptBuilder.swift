...
enum PromptBuilder {
    ...
    **analysis 使用者永遠看不到**——它是給系統和你下一輪的自己看的。
    ...
    static func roundDirective(_ round: Int, hasNote: Bool = false) -> String {
        ...
        case 3:
            ...
            `### ` 標題行點出重點，再一個 `- ` 項目給一個現在做得到的方向。加起來不准超過 110 字。
        ...
        case 5:
            ...
            **可以繼續處理目前的問題**，但同時要開始整理已知資訊、減少開啟新議題，\
            把討論導向一個具體方向而不是繼續發散。目標長度 100 字以內。\
        ...
        case 7:
            ...
            **不主動開啟任何新的大型議題**。整理目前最重要的理解、給出具體下一步、準備結束這次 session。\
            語氣要讓使用者感覺到「快收尾了」但不是被趕。目標長度 90 字以內。\
        ...
        default:
            ...
            **不再提出新的探索問題、不開新議題、不做新的長篇分析、不推薦新的複雜策略。**
            ...
            ③ 一個最值得採取的 Next Step（用 `- ` 寫）④ 清楚但溫和地結束。全部 110 字以內。
    ...
    - text：必填，繁體中文。**硬性上限 130 字**——這是手機上的聊天泡泡，不是文章。
      超過 130 字的回覆一律視為錯誤輸出。寧可少講也不要超過。
    ...
    **標題和項目不可以講同一句話**。標題說「要做什麼」，項目說「具體怎麼做」。
    ...
    排版不能拿來換字數——用了標題和項目符號就要把句子刪得更短，總量仍是 130 字以內。

    ## 自我檢查（輸出前默唸一次，做完再輸出）
    1. 數 text 的字數，超過 130 了嗎？→ 刪掉最不重要的那一句，只留最有用的。
