...
struct TaskCardView: View {
    ...
    var body: some View {
        HStack(spacing: 0) {
            // 分類色條貼齊卡片左緣，當成邊緣裝飾而不是內容的一部分。
            // 之前色條 5pt + 間距 14pt 都算在內距裡，內容等於被往右推 19pt，
            // 右邊卻只有 16pt，整張卡看起來就往右偏。
            Rectangle()
                .fill(categoryColor)
                .frame(width: 5)
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                // 操作按鈕列：已拆分母任務唯讀，動作只剩標題列的「…」→ 查看任務記錄（SPL-05）
                if isSplitParent {
                    Text("已拆分成子任務")
                        .font(.alpacaCaption)
                        .foregroundStyle(.secondary)
                } else {
                    // 開始/拆分 ｜ 卡住了 ｜ 完成 —— 「…」已經移到標題列
                    // 「卡住了」是整個 App 的 AI 入口，藏在「…」裡會低估這個功能。
                    // 全部 lineLimit(1)+fixedSize()：寧可整列擠，也不要中文標籤折成兩行把卡片撐高。
                    HStack(spacing: 10) {
                        // 主要按鈕：開始 →（開始後）拆分
                        ...
                        Button {
                            ...
                            Label("卡住了", systemImage: "exclamationmark.bubble.fill")
                                .font(.subheadline)
                                .lineLimit(1)
                                .fixedSize()
                                // tint 只負責底色。alpacaStuck 在 B 的色盤是淡薰衣草，
                                // 直接當文字色會太淺，所以字和 icon 另外指定深色。
                                .foregroundStyle(Color.alpacaBrown)
                        }
                        .buttonStyle(.bordered)
    ...
    /// 一般卡片的「…」：編輯／查看任務記錄／刪除，還沒開始時多一個拆分。
    /// 「卡住了」已經拉到按鈕列，這裡不重複放。
    /// 拆分只在還沒開始時出現——開始之後主要按鈕本身就是「拆分」。
